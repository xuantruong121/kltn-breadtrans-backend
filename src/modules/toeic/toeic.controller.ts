...
export class ToeicController {
  ...
  startAttempt(
    ...
    return this.toeicService.startAttempt(req.user.id, +examId, mode || AttemptMode.PRACTICE);
  }
  ...
  saveAnswers(@Param('id') id: string, @Body('answers') answers: Record<string, number>) {
